...
fn main() {
    ...
    println!("cargo:info=Target OS: {}", env::var("CARGO_CFG_TARGET_OS").unwrap());
    ...
    if env::var("CARGO_CFG_TARGET_OS").unwrap() == "windows" {
        ...
        // Define paths
        let source_path = manifest_dir.join("bindings").join("windows").join("monitor.c");
        let output_dir = manifest_dir.join("bindings").join("windows").join("release");
